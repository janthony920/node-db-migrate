...
function writeMigrationRecord(db, migration, callback) {
  db._runSql('INSERT INTO ? (name, run_on) VALUES (?, ?)', [MIGRATION_TABLE_NAME, migration.name, new Date()], callback);
}
...
Migration.loadFromDatabase = function(dir, driver, callback) {
  ...
  console.log(MIGRATION_TABLE_NAME);
  console.log('....');
  var selectSql = 'SELECT * FROM ' + MIGRATION_TABLE_NAME + ' ORDER BY name DESC';
  driver.all(selectSql, function(err, dbResults) {
    if (err) { callback(err); return; }
