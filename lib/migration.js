...
var migrationTableName = require('./driver/base').migrationTableName;
...
function writeMigrationRecord(db, migration, callback) {
  db._runSql('INSERT INTO migrations2 (name, run_on) VALUES (?, ?)', [migration.name, new Date()], callback);
}
...
Migration.loadFromDatabase = function(dir, driver, callback) {
  ...
  console.log('....');
  driver.all('SELECT * FROM migrations3 ORDER BY name DESC', function(err, dbResults) {
    if (err) { callback(err); return; }
